...
#  НАСТРОЙКИ КАНАЛА
# ═══════════════════════════════════════════════════════════════
CHANNEL_NAME = "Цифровая звонница"          # отображается на плашке картинки
CHANNEL_HANDLE = "@digitalzvon"      # замени на свой
...
SOURCES = {
    ...
    "Фото": [
        ("Pinterest дизайн",  "https://rss.app/feeds/RjqtSq710RR98Wns.xml"),
...
DAILY_CATEGORIES = {
    ...
    3: ["ИИ в дизайне"],
    4: ["Москва события"],
    5: ["Туториалы"],   # суббота: утром — разбор, вечером — дайджест
}
...
DAY_HASHTAGS = {
    ...
    5: "#разбор #туториал #инструменты",
}

# Цвет плашки по рубрике (для визуальной рамки)
CATEGORY_COLOR = {
    "Типографика":          (220, 50, 47),
    "Брендинг":             (211, 54, 130),
    "Интерфейсы":           (38, 139, 210),
    "3D и моушен":          (108, 113, 196),
    "Индустриальный дизайн":(133, 153, 0),
    "Студии и блоги":       (181, 137, 0),
    "Искусство":            (203, 75, 22),
    "Фото":                 (42, 161, 152),
    "ИИ в дизайне":         (88, 110, 117),
    "Москва события":       (220, 50, 47),
    "Туториалы":            (38, 139, 210),
}
...
# ═══════════════════════════════════════════════════════════════
def ensure_font():
    """Скачивает Inter если нет. Фоллбэк на DejaVu / дефолт."""
    if Path(FONT_PATH).exists():
        return
    # Пробуем Inter из GitHub
    try:
        import zipfile
...
def get_font(size, bold=True):
    ...
    if not Path(path).exists():
        path = FONT_PATH  # фоллбэк на bold
    try:
...
# ═══════════════════════════════════════════════════════════════
#  КАРТИНКА: извлечение + 16:9 + визуальная рамка
# ═══════════════════════════════════════════════════════════════
...
def add_visual_frame(img, category, source=""):
    """Добавляет брендированную рамку: градиент снизу, цветная линия сверху, текст."""
...
def make_cover_image(image_url, category="", source=""):
    """Скачивает, кропает 16:9, добавляет рамку. Возвращает bytes JPEG."""
    try:
...
def generate_friday_card(title, events):
    """Генерирует карточку-подборку для пятничных событий."""
    try:
...
# --- Голос канала (встроен во все промпты) ---
VOICE_RULES = """
ГОЛОС КАНАЛА:
...
3. Украсть: один приём, подход или инструмент, который можно забрать себе. Начни со слова действия («Попробуйте», «Обратите внимание», «Возьмите на вооружение» — но разнообразь).
4. Если уместно, одним предложением свяжи с более широким трендом («Третий кейс за месяц, где...», «Это перекликается с...»).

ПРАВИЛА:
- Не выдумывай фактов. Имена, цифры — ТОЛЬКО из источника.
- Общая длина: до 550 символов.

JSON БЕЗ ```:
{{"title": "...", "summary": "...", "steal": "...", "trend": "..."}}
...
2. На каждый материал: суть в одном предложении + чем полезно дизайнеру (один конкретный вывод).
3. Если между материалами есть связь или общий тренд — упомяни одним предложением в конце.

JSON БЕЗ ```:
{{"title": "...", "items": [{{"name": "...", "summary": "...", "value": "..."}}], "trend": "..."}}
...
def post_to_telegram(token, chat_id, text, image_bytes=None):
    """Отправляет пост. image_bytes — готовый JPEG."""
